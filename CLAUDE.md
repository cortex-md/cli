...
apps/cli/
  ...
  internal/
    ...
    publish/            # Shared local publish artifact metadata and packaging
...
- keep CLI colors in `internal/ux`
- use the Cortex octopus red (`#FF3B38`) for brand marks and active steps
- use the Cortex accent gold (`#E8A83C`) for command highlights and informational markers
- use semantic project colors for success, warning, and error states
- root and compact banners should keep the ASCII octopus mark as the primary brand signal
...
- local publish does not resolve tokens
- registry submit resolves token from `CORTEX_TOKEN`, then `GITHUB_TOKEN`, then keychain from `cortex login`

...
func Start(pluginDir string, opts DevOptions) error {
	...
	if !opts.SkipLink {
		linkResult, err = Link(absDir, LinkOptions{VaultPath: opts.VaultPath})
		if err != nil {
	...
	if linkResult != nil {
		session.pluginID = linkResult.ID
...
func (s *DevSession) notifyReload() {
	if s.linkPath == "" {
		ux.Info("Plugin rebuilt - link it to a vault with cortex plugin link --vault /path/to/vault")
		return
	}
	if err := TouchDevReloadMarker(s.reloadDir, s.pluginID); err != nil {
		ux.Warning("Plugin rebuilt, but reload notification failed: %v", err)
		return
	}
	ux.Info("Plugin rebuilt - Cortex will reload it when the vault is open")
}
...
func (s *DevSession) cleanup() {
	...
	if s.linkPath != "" {
		if err := RemoveDevLink(s.linkPath, s.linkTarget, s.reloadDir, s.pluginID); err != nil {
			ux.Warning("Could not remove dev plugin link: %v", err)
			return
		}
		ux.Info("Removed dev plugin link: %s", s.linkPath)
	}
}
